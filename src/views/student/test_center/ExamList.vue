<template>
  <div class="test-center-container">
    <div class="header">
      ...
      <el-button type="primary" plain @click="goToResultList">查看测验结果</el-button>
    </div>
...
function seedExamTimerMeta(paper) {
  if (typeof window === 'undefined') return
  const metaKey = buildTimerMetaKey(paper.id)
  const now = Date.now()
  const meta = {
    paperId: paper.id,
    title: paper.paperName || paper.title || '未命名试卷',
    timeLimitSeconds: Math.max(0, Number(paper.timeLimit || 0) * 60),
    deadlineTime: paper.deadline || null,
    startTime: now,
    lastHeartbeat: now
  }
...
function buildTimerMetaKey(paperId) {
  const userKey = userStore.userKey || 'guest'
  return `exam_timer_meta_${userKey}_${paperId}`
}
